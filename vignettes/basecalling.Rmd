...
Basecalling on a remote server is slightly more complicated. This script shows how to do this on a SLURM server with a GPU node. The `remote_dir` must contain a job submission script (based on this [template](https://github.com/markschl/DadaNanoBC/blob/main/templates/base_calling/slurm_submit_basecaller.sh)); it can be copied to the remote directory e.g. using an FTP client; adjust the `dorado` path and your email address inside the script.
