...
Inconsistent taxonomic labels are flagged in the report, and potential **contaminants** are automatically down-ranked in the mix if (i) the names were blacklisted in `config.yaml`, or (ii) there exists a less abundant taxon, which has the correct kingdom, or it has at least *three more consistent (matching) taxonomic ranks* in the GBIF lineage compared to the top taxon (undefined ranks excluded from the comparison).
