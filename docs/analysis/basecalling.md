...
If not using Dorado for other things, you may also use the `install-dorado.sh` installation script, which automatically downloads the software into the *bin* subfolder (Linux / Mac only).
...
# replace 'dorado' with full path to the 'dorado' binary if necessary; e.g.
