...
amplicon_stats = sample_tab %>% 
  ...
  summarise(
    `# samples` = n(),
    `# with morphological identification` = sum(!is.na(morphospecies)),
    `# with known sequence` = sum(!is.na(`known sequence`)),
    .groups = 'drop'
  )
```
...
> *Note*: Using a lower `idx_max_diffs` threshold reduces false assignments of 
> reads to samples, but also leads to less sequences being included in the analysis.
> On the other hand, an increased error tolerance increases the read count.
> You can check the unused barcode combinations (*unspecific* in below figures)
...
ggplot(idxdist, aes(dist)) +
  ...
  facet_grid(amplicon ~ direction, scales='free') + 
...
demux_dir <- file.path(analysis_dir, 'tmp', 'demux')
...
# stats
tail_len_mean = primer_stats$tail_len %>% 
    group_by(amplicon, primer_dir) %>% 
    summarise(tail_len = weighted.mean(tail_len, count),
              .groups='drop')
...
mean_len = mean(na.omit(amplen_mean$mean_len))

qual_stats = primer_stats$quality %>% 
  mutate(
    primer_mis_grp = ifelse(!is.na(primer_mis) & primer_mis < 4, as.character(primer_mis), '4+'),
    bc_mis_grp = ifelse(!is.na(bc_mis) & primer_mis < 2, as.character(primer_mis), '2+')
  )
```
...
A small part of the reads has tail lengths around the mean fragment length (or multiples of it), which indicates the presence of concatenated artifacts (Figures \@ref(fig:tail-stats), \@ref(fig:len-stats)). 

(ref:tail-stats) Distribution of the primer trimmed tail lengths (*note the non-linear scale*)

```{r tail-stats, fig.width=6.7, fig.height=3, fig.cap='(ref:tail-stats)', warning=F}
ggplot(primer_stats$tail_len, aes(tail_len)) +
  geom_density(aes(weight=count), fill='#a6bddb', color='#2b8cbe', adjust=.1) +
  facet_grid(primer_dir ~ amplicon, scales='free') +
  scale_y_sqrt(breaks = 10^(-4:0), expand=expansion(mult=c(0, .05))) +
  scale_x_continuous(limits=c(0, 1.4*mean_len), expand=expansion()) +
...
ggplot(d, aes(length, fill=category, color=category)) +
  ...
  scale_y_continuous(expand=expansion(mult=c(0, .05))) +
  scale_x_continuous(limits=c(0, 2.5*mean_len), expand=expansion()) +
...
The quality filtering should remove highly errorneous sequencing reads, while still retaining enough reads for inferring the barcode sequences. The default threshold of *2.5 expected errors per read* can be tweaked (`error_threshold` setting, use Figure \@ref(fig:readstats) for guidance).

(ref:readstats) Read counts by category

```{r readstats, fig.width=6.7, fig.height=2.2, fig.cap='(ref:readstats)'}
d <- primer_stats$primers %>% 
  group_by(category, valid) %>% 
  summarise(count = sum(count), .groups='drop')

ee = qual_stats %>% 
  filter(!is.na(bc_mis) & !is.na(primer_mis)) %>% 
  group_by(category = paste(cut(exp_err, c(-1:5, Inf)), 'errors')) %>% 
  summarise(
    count = sum(count)
  )
stopifnot(d$count[d$valid] == sum(ee$count))
...
d.ee = bind_rows(
  ...
  mutate(retain = ifelse(valid, 'valid amplicon', 'no primer(s) /\nfusion artifact'))

ggplot(d.ee, aes(retain, count, fill=category)) +
...
According to Figure \@ref(fig:mismatch-stats), reads with many primer or sample index mismatches also tend to have lower quality scores. With the current setting (`error_threshold`), reads with >`r error_threshold` errors and reads shorter than `r (opts$demultiplex$min_read_length %||% 50)` bp (`min_read_length` setting) are discarded.
...
p1 = qual_stats %>%
  group_by(bc_mis, primer_mis) %>% 
  summarise(exp_err = weighted.mean(exp_err, count), .groups='drop') %>% 
  ggplot(aes(bc_mis, primer_mis, fill=exp_err)) +
  geom_tile() +
  ...
  theme(legend.position='bottom') +
  labs(x='Barcode mismatches', y='Primer mismatches',
...
p3 = qual_stats %>%
  group_by(exp_err=cut(exp_err, c(-1:5, Inf)), bc_mis_grp) %>% 
  summarise(count = sum(count), .groups='drop') %>% 
  ggplot(aes(exp_err, count, fill=as.factor(bc_mis_grp))) +
  geom_col() +
  count_plot_cfg +
  labs(fill='Barcode mismatches')
...
primer_search_fq <- file.path(demux_dir, amplicons, 'demux.fastq.zst')
args <- list(
...
Before the DADA2 denoising, it is necessary to determine how the quality scores in the sequencing reads correlate with the substitution error rates. Usually, the relationship looks fairly log-linear, although there can be a plateau/increase at Q > 40 (Figure \@ref(fig:errplot)).
...
```{r errplot, fig.width=4.5, fig.height=3.8, fig.cap='(ref:errplot)', warning=F}
...
The DADA2 denoising method relies on presence of **at least a few error-free reads** in the filtered Nanopore data. This should usually be the case with recent (R10.4) sequencing data, and assuming that amplicons are not extremely long. Specifically, the proportion of singletons^[Singletons are sequences only present *once* in a sample (non-duplicated)] has to be lower than 100 % (Figure \@ref(fig:cluster-validation)b), as duplicated sequences are likely to represent true sequences in the sample, while singletons often contain sequencing errors.
...
```{r cluster, include=F}
aln_dir = file.path(analysis_dir, 'output', 'alignments')
...
(ref:polymorphisms) (a/b) Distribution of putative haplotype numbers for the top organism in a sample; (c) Proportion of reads from the *top* organism group relative to the total analyzed read depth
...
```{r load-taxdb}
taxdb_file <- do.call(load_taxdb, c(list(db_dir='taxdb'), opts$taxonomy))
```
...
if (any(seq_tab$has_contamination)) {
  ...
  rownames(cont) <- NULL
  cont %>% 
    select(sample, morphospecies, `correct taxon`=top_seq_taxon, contaminants) %>% 
    mutate(sample = stringr::str_trunc(sample, 30))
}
```

(ref:tax-overlap) Overview of taxonomic assignments, summarized per plate. (a) Taxonomic composition (based on barcode sequences); (b) Taxonomic overlap between the morphological and sequence-based taxonomic assignments

```{r tax-overlap, warning=FALSE, fig.width=6.7, fig.height=3 + 0.9 * sqrt(length(unique(seq_tab$plate))), fig.cap='(ref:tax-overlap)'}
abundant.lineages = names(head(sort(table(seq_tab$top_seq_short_lineage), TRUE), 10))
d <- seq_tab %>% 
  select(amplicon, plate, lineage = top_seq_short_lineage,
  ...
  mutate(
    ...
    lineage = factor(
      case_when(
        lineage %in% abundant.lineages ~ lineage,
        n_seqs > 0 ~ 'Other',
        n_seqs == 0 ~ '(no sequence)'
      ),
      c(abundant.lineages, 'Other', '(no sequence)')
    )
  )
p1 = ggplot(d, aes(as.factor(plate), fill=lineage)) +
  geom_bar() +
  facet_grid(amplicon ~ ., scales='free') +
...
p2 <- ggplot(d.ov, aes(tax_overlap, fill=category)) +
  geom_histogram(binwidth=1/7) +
  facet_wrap(~ sprintf('%s / p%d', amplicon, plate), scales='free_y') +
...
(wrap_elements(p1) / p2) +
  plot_layout(heights = c(3, 1.5 * sqrt(length(unique(seq_tab$plate))))) +
  plot_annotation(tag_levels='a')
```
...
if (any(sel)) {
  t <- run_clustering(
    ...
    dada_err,
    cache_file = file.path(analysis_dir, 'tmp/cluster_contam'),
    aln_dir = aln_dir,
    cores = cores,
    opts = modifyList(opts$cluster %||% list(), list(max_sample_depth = 1e7)),
    overwrite = FALSE
  )
  tax_prefix <- file.path(analysis_dir, 'tmp/taxonomy_cont')
  args <- c(list(seq_tab[sel, ], tax_prefix, taxdb_file), opts$taxonomy)
...
# TODO: cache?
seq_tab <- compare_known_seqs(seq_tab)
```
